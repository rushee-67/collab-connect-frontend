...
const MeetingRoom = () => {
  ...
  useEffect(() => {
    if (localStream && localVideoRef.current) {
      ...
      // ensure play attempt after user interaction
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="flex h-[calc(100vh-80px)]">
        <div className="flex-1 p-4">
          <div className="mb-4 flex justify-between items-center bg-gray-800 p-4 rounded">
            ...
            <button onClick={handleLeaveMeeting} className="px-6 py-2 bg-red-600 rounded hover:bg-red-700">
              Leave Meeting
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="relative bg-gray-800 rounded-lg overflow-hidden aspect-video">
              ...
              <div className="absolute bottom-2 left-2 bg-black bg-opacity-50 px-2 py-1 rounded">You ({userName})</div>
            </div>
          ...
          {connectionStatus !== 'connected' && <div className="mt-4 p-4 bg-yellow-800 rounded">Connection Status: {connectionStatus}</div>}
          {error && <div className="mt-4 p-4 bg-red-800 rounded">Error: {error}</div>}
        </div>

        <div className="w-96 bg-gray-800 flex flex-col">
          <div className="p-4 border-b border-gray-700"><h2 className="text-xl font-semibold">Chat</h2></div>
          ...
          <div className="p-4 border-t border-gray-700">
            <div className="flex gap-2">
              <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="Type a message..." className="flex-1 px-3 py-2 bg-gray-700 rounded focus:outline-none focus:ring-2 focus:ring-blue-500" />
      ...
      <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 flex gap-4">
        <button onClick={toggleAudio} className={`px-6 py-3 rounded-lg font-semibold ${isAudioEnabled ? 'bg-gray-700 hover:bg-gray-600' : 'bg-red-600 hover:bg-red-700'}`}>{isAudioEnabled ? 'Mute Mic' : 'Unmute Mic'}</button>

        <button onClick={toggleCamera} className={`px-6 py-3 rounded-lg font-semibold ${isVideoEnabled ? 'bg-gray-700 hover:bg-gray-600' : 'bg-red-600 hover:bg-red-700'}`}>{isVideoEnabled ? 'Turn Off Cam' : 'Turn On Cam'}</button>
